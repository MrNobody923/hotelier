...
$requiredFields = ['id', 'item_name', 'price'];
$allPresent = true;
...
if ($data && $allPresent) {
    try {
        ...
        $stmt = $conn->prepare("UPDATE menu_items SET item_name = ?, category = ?, description = ?, price = ? WHERE id = ?");
        if (!$stmt) {
             throw new Exception("Prepare failed: " . $conn->error);
        }
        $stmt->bind_param("sssdi", $item_name, $category, $description, $price, $id);

        if ($stmt->execute()) {
            echo json_encode(["status" => "success", "message" => "Menu item updated!", "saved_category" => $category]);
        } else {
    ...
    echo json_encode(["status" => "error", "message" => "Incomplete data or missing ID. Missing: " . implode(", ", array_diff($requiredFields, array_keys($data)))]);
}
$conn->close();
?>
